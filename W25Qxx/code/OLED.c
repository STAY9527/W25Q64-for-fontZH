...
void OLED_DisplayLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,OLED_ColorMode color){
  ...
  if (x1 == x2){
    ...
    for (uint8_t y = y1; y <= y2; y++){
        // OLED_GRAM[y/OLED_PAGE][x1] |= 0x01 << (y % OLED_PAGE);
        OLED_DisplayPixel(x1, y,color);
    }
  }
  else if (y1 == y2){
    if (x1 > x2){
    ...
    for (uint8_t x = x1; x <= x2; x++){
        // OLED_GRAM[y1/OLED_PAGE][x] |= 0x01 << (y1 % OLED_PAGE);
      OLED_DisplayPixel(x, y1,color);
    }
  }
  else{
    // Bresenham直线算法
    ...
    if (dx > dy){
      for (x = x1; x != x2; x += ux){
        // OLED_GRAM[y/OLED_PAGE][x] |= 0x01 << (y % OLED_PAGE);
        OLED_DisplayPixel(x, y,color);
        eps += dy;
        if ((eps << 1) >= dx){
          y += uy;
          eps -= dx;
...
void OLED_DisplayRectangle(uint8_t x, uint8_t y, uint8_t w, uint8_t h,OLED_ColorMode color){
  OLED_DisplayLine(x, y, x + w, y,color);
  OLED_DisplayLine(x, y + h, x + w, y + h,color);
  OLED_DisplayLine(x, y, x, y + h,color);
  OLED_DisplayLine(x + w, y, x + w, y + h,color);
  // OLED_ShowFrame();
}
...
void OLED_DisplayFilledRectangle(uint8_t x, uint8_t y, uint8_t w, uint8_t h,OLED_ColorMode color){
  for (uint8_t i = 0; i < h; i++){
    OLED_DisplayLine(x, y + i, x + w, y + i,color);
  }
  // OLED_ShowFrame();
}
...
void OLED_DisplayTriangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t x3, uint8_t y3, OLED_ColorMode color){
  OLED_DisplayLine(x1, y1, x2, y2, color);
  OLED_DisplayLine(x2, y2, x3, y3, color);
  OLED_DisplayLine(x3, y3, x1, y1, color);
  // OLED_ShowFrame();
}
...
void OLED_DisplayFilledTriangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t x3, uint8_t y3, OLED_ColorMode color){
  ...
  for (; y <= b; y++){
    if (y <= y3){
      OLED_DisplayLine(x1 + (y - y1) * (x2 - x1) / (y2 - y1), y, x1 + (y - y1) * (x3 - x1) / (y3 - y1), y, color);
    }
    else{
      last = y - 1;
      break;
  ...
  for (; y <= b; y++){
    OLED_DisplayLine(x2 + (y - y2) * (x3 - x2) / (y3 - y2), y, x1 + (y - last) * (x3 - x1) / (y3 - last), y, color);
  }
  // OLED_ShowFrame();
}
...
void OLED_DisplayCircle(uint8_t x, uint8_t y, uint8_t r, OLED_ColorMode color){
  ...
  while (a <= b){
    OLED_DisplayPixel(x - b, y - a, color);
    OLED_DisplayPixel(x + b, y - a, color);
    OLED_DisplayPixel(x - a, y + b, color);
    OLED_DisplayPixel(x - b, y - a, color);
    OLED_DisplayPixel(x - a, y - b, color);
    OLED_DisplayPixel(x + b, y + a, color);
    OLED_DisplayPixel(x + a, y - b, color);
...
void OLED_DisplayFilledCircle(uint8_t x, uint8_t y, uint8_t r, OLED_ColorMode color){
  ...
  while (a <= b){
    for (int16_t i = x - b; i <= x + b; i++){
      OLED_DisplayPixel(i, y + a, color);
      OLED_DisplayPixel(i, y - a, color);
    }
    for (int16_t i = x - a; i <= x + a; i++){
      OLED_DisplayPixel(i, y + b, color);
      OLED_DisplayPixel(i, y - b, color);
    }
    a++;
    if (di < 0){
...
void OLED_DisplayEllipse(uint8_t x, uint8_t y, uint8_t a, uint8_t b, OLED_ColorMode color){
  ...
  while (a2 * ypos > b2 * xpos){
    OLED_DisplayPixel(x + xpos, y + ypos, color);
    OLED_DisplayPixel(x - xpos, y + ypos, color);
    OLED_DisplayPixel(x + xpos, y - ypos, color);
    OLED_DisplayPixel(x - xpos, y - ypos, color);
    if (d < 0){
...
// uint8_t data[1152];
// W25QXX_Read(data, 0x000000, 1152)
...
void OLED_PrintString(uint8_t x, uint8_t y, char *str, OLED_ColorMode color)
{
  ...
  uint8_t oneLen = 36; // 一个字模占多少字节
  uint8_t found;                                        // 是否找到字模
  uint8_t utf8Len;                                      // UTF-8编码长度
  uint8_t *head;  
  uint8_t Data[PAGE_FONT_SIZE*36];                                      // 字模头指针
  while (str[i])
  {
    found = 0;
    ...
    for (uint32_t page=0; page<NUMBER_FONT_SIZE/PAGE_FONT_SIZE; page++){
      W25QXX_Read(Data,page*PAGE_FONT_SIZE*36,PAGE_FONT_SIZE*36);//
      for (uint16_t j = 0; j < PAGE_FONT_SIZE; j++)
      {
        head = (uint8_t *)Data + (j * oneLen);
        if (memcmp(str + i, head, utf8Len) == 0)
        {
          // OLED_SetBlock(x, y, head + 4, font->w, font->h, color);
          OLED_SetBlock(x, y, head + 4, 16, 16, color);
          // 移动光标
          // x += font->w;
          x += 16;
          i += utf8Len;
    ...
    if (found == 0)
    {
      ...
      if (utf8Len == 1)
      {
        ...
        OLED_PrintASCIIChar(x, y, str[i], &afont16x8, color);
        // 移动光标
        x += 8;
        i += utf8Len;
      }
      ...
      {
        OLED_PrintASCIIChar(x, y, ' ', &afont16x8, color);
        x += 8;
        i += utf8Len;
      }
    }
