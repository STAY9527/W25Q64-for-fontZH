...
uint16_t W25QXX_ReadID(void)
{
    ...
    uint8_t send_data[4] = {ManufactDeviceID_CMD,0x00,0x00,0x00};   //待发送数据，命令+地址(0x000000h)
    
    /* 使能片选 */
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
    
    /* 发送并读取数据 */
    if (HAL_OK == SPI_Transmit(send_data, 4)) //确认是否发送成功
    {
        if (HAL_OK == SPI_Receive(recv_buf, 2)) //确认是否接收成功
        {
            device_id = (recv_buf[0] << 8) | recv_buf[1]; //将两个8位数据拼接成一个16位数据,recv_buf[0]为设备ID位(16h),recv_buf[1]为制造商ID(EFh)
        }
    }
 ...
 * @brief   读取SPI FLASH数据
 * @param   buffer      —— 数据存储区
 * @param   start_addr  —— 开始读取的地址(最大32bit)0x7fffffh
